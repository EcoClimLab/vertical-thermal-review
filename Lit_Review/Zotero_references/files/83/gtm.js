...
0<l.length&&(k=l[0])}else k=H.getElementById(b.vtp_elementId);if(k)switch(h){case "BOOLEAN":var m=(Number(b.vtp_onScreenRatio)||50)/100;e.value=Gk(k)>=m&&!Hk(k);break;case "PERCENT":e.value=0,Hk(k)||(e.value=Math.round(1E3*Gk(k))/10)}a[c]=e;return e.value})}();
