...
SCM.Logging = (function() {
    var logger = {
        // Note that the logger does not have a log() function, because we should never be doing a console.log()
        debug: _debug,
        warn: _warn,
        error: _error
    };
    var jsDebugEnabled = null;


    return {
        init: _init,
        getLogger: function () { return logger; },
        isJsDebugEnabled: _isJsDebugEnabled
    };

    function _debug(msg) {
        _isJsDebugEnabled() && console.log(msg);
    }
    ...
    function _isJsDebugEnabled() {
        var jsDebugParam;
        return jsDebugEnabled !== null
            ? jsDebugEnabled
            : jsDebugEnabled = (jsDebugParam = SCM.JSUtility.getParameterByName('JsDebug', window.location.href))
            && jsDebugParam === 'true';
    }
...
// If enabled, log any warnings generated by jquery-migrate for the purpose of identifying and fixing issues

var SCM = SCM || {};
SCM.JqueryMigrateLogger = function() {
    var pollTimeInMs = 5000;
    var pollLimit = 20;
    var pollCount = 0;
    var $loggingDetailElement;
    var logger;
    ...
    function _init() {
        logger = SCM.Logging.getLogger();
        if (!_isLoggingEnabled()) {
            logger.debug('jquery-migrate logging is disabled');
            return;
        }

        logger.debug('jquery-migrate logging is enabled.  Attempting to initialize Application Insights.');
        var instrumentationKey = _getInstrumentationKey();
        if (!instrumentationKey) return;
        _initApplicationInsights(instrumentationKey);
        if (!window.appInsights) {
            logger.warn('Application Insights failed to initialize.');
            return;
        }
        setTimeout(_logWarnings, pollTimeInMs);
    }

    function _getInstrumentationKey() {
        if (!$loggingDetailElement && !$loggingDetailElement.length) {
            return '';
        }
        var key = $loggingDetailElement.data('jqueryMigrateInstrumentationKey');
        return key ? key.trim() : '';
    }

    function _initApplicationInsights(instrumentationKey) {
        ...
        window[aiName] = aisdk;  // [We don't want to track page views.]  aisdk.queue && 0 === aisdk.queue.length&&aisdk.trackPageView({});
    }

    function _isLoggingEnabled() {
        $loggingDetailElement = $('[data-jquery-migrate-logging-enabled]');
        return $loggingDetailElement && $loggingDetailElement.length && jQuery.migrateWarnings;
    }

    function _logWarnings() {
        if (jQuery.migrateWarnings && jQuery.migrateWarnings.length) {
            logger.debug('Logging ' + jQuery.migrateWarnings.length + ' jquery-migrate warnings.');
            jQuery.migrateWarnings.forEach(function (w) {
                logger.debug(w);
	...
	$.deparam.querystring = function () {
		...
		for (var i = 0; i < terms.length; i++) {
			...
			if (keyValue.length === 2) {
                toRet[keyValue[0]] = decodeURI(keyValue[1]);
			}
		}
...
(function (bc, $) {
    bc.init = function () {
        if ($('.brightcove-wrapper video.video-js, .brightcove-player').length !== 0) {
            ...
            jsSrc = "//players.brightcove.net/1579648260001/default_default/index.min.js";
            $.getScript(jsSrc, function () {
            });
        }
...
(function (ns, $) {
    ...
    ns.trackBookPageEvents = function () {
        var $doc = $(document);
        // pdf
        $doc.on('click', '.widget-BookChapterMainView a.article-pdfLink', function () {
            // NARN-12168, reasoning for this change:
            // * Change to NTPT_PGEXTRA only happens upon click event of a chapter PDF download
            // * Values within NTPT_PGEXTRA are all set upon a chapter page load (Search for NTPT_PGEXTRA in HTML of chapter page)
            // * Click event will replace "event_type" to desired type, "full-text.pdf"
            // * Pressing back from the loaded chapter PDF page will reload the NTPT_PGEXTRA and display correct type
            NTPT_PGEXTRA = NTPT_PGEXTRA.replace('event_type=full-text', 'event_type=full-text.pdf');
            ns.trackOnPageEvent('full-text.pdf');
        });
    }
    ...
    $(function () {
        ...
        if ($('.pg_ChapterStandard').length) {
            ns.trackBookPageEvents();
        }
    });
...
(function (ns, $) {
        ...
        (function () {
            ...
            SCM.SplitScreen.toggleSplitView = function (override) {
                ...
                if (!splitView) {
                    ...
                    var $splitViewFlyoutIcons = $('.widget-SplitView .citation-links-icon');

                    $splitViewFlyoutIcons.addClass('hide');
    ...
    SCM.SplitScreen.WidgetStore.register({
        ...
        render: function (ArticleFulltext) {
            return createElement('div', { 'class': 'dynamic-widget-inner-wrap' },
            getElementsByClassList(ArticleFulltext, [
                'paragraph-ref',
                'ref-list']));
        }
    });
    ...
    SCM.SplitScreen.WidgetStore.register({
        ...
        render: function (ArticleFulltext, ArticleTopInfo, ArticleLinks, Toolbar, ArticleDataSupplements) {
            ...
            var authorFlyouts = $('.al-author-info-wrap');
            function hideAuthorFlyouts() {
                authorFlyouts.removeClass('open');
            }
            ...
            for (var j = 0; j < flyouts.length; j++) {
                flyouts[j].addEventListener('click', function (e) {
                    ...
                    var authorCard = $(e.target).parent('.js-flyout-wrap').find('.al-author-info-wrap');

                    $('.al-author-info-wrap').not(authorCard).removeClass('open');

                    authorCard.toggleClass('open').attr('tabindex', '-1').focus();

                    $('.al-author-info-wrap').not(authorCard).removeClass('open');

                    $(document.body).on('click', function (e) {
                        if (!$(e.target).parents('.al-author-info-wrap').length) {
                          
                            hideAuthorFlyouts();                        }
                    });

                    // reposition authorCards that would get cut off by right pane
                    var authorLinkWrap = $(e.target).closest('.al-author-name'); // We need the position of the link's parent, not the link itself
                    console.log(authorLinkWrap);
                    var linkPos = authorLinkWrap.offset(); 
                    console.log(linkPos);
                    
                    // 320 is the width of the authorCard
                    if (linkPos.left > halfWidth + 320) {
                        authorCard.addClass('shift-card'); 
                    }
    ...
    SCM.SplitScreen.WidgetStore.register({
        ...
        render: function () {
            return createElement('li', { 'class': 'toolbar-item item-link' }, [
                createElement('a', { 'class': 'standard-view', href: 'javascript:;', 'onclick': SCM.SplitScreen.toggleSplitView }, [
...
(function (ns, $) {
    ns.ChapterEndNotes = function () {
        $('.js-related-object[object-type="end-note"]').on('click', function myFunc() {
            var legacySectionId = $(this).attr('legacysectionid');
            var $displaydiv = $('.js-footnote-modal[legacysectionid= "' + legacySectionId + '"]');
            var $footnote = $('div[content-id = "' + legacySectionId + '"]');

            if ($footnote != null) {
                var $paragraph = $footnote.find('.fn');
            }

            if ($paragraph != null) {
                var $revealContent = $displaydiv.find('#revealContent');

                if ($revealContent.length && $revealContent.html() === "") {
                    $revealContent.append(($paragraph).clone());
                }
            }              

            // Scroll to the top the modal
            $displaydiv.foundation('reveal', 'open');
            if ($displaydiv != null && $displaydiv.length) {
                $(window).scrollTop($displaydiv.offset().top - 50);
            }
        });
    }
    ns.init = function(){
...
SCM.QueryBuilder.configure = function () {
    ...
    (function ($) {
        ...
        $('.querybuilder-instance').each(function () {
            if (!$(this).hasClass('hide')) {
                $visibleInstances++;
                var $operatorValue = $(this).find("[id^=operator-]");
                if ($operatorValue !== undefined && $operatorValue.length > 0) {
                    $(this).find("[id^=" + $operatorValue.val().toLowerCase() + "Op-]").prop("checked", true);
                }
            }
        });
    ...
    $queryAdd.on('click', function () {
        ...
        if ($(this).attr("disabled")) {
            ...
            var $operatorsInstanceID = $("#operators-qbInstance" + (nextInstanceNum));
            if ($operatorsInstanceID !== undefined && $operatorsInstanceID.length > 0) {
                // show next QB instance & disable button
                $operatorsInstanceID.removeClass('hide');
                var $andOp = $("#andOp-qbInstance" + (nextInstanceNum));
                $andOp.prop("checked", true);
            }            
    ...
    $('.js-main, .js-querybuilder-wrap').off('click').on('click', '.querybuilder-submit', function () {
        ...
        var queryString = "";
        if ($('.js-custom-queryBuilder').length > 0) {
            queryString = SCM.CustomQueryBuilder.queryBuilderHandler();
        }
        else {
            ...
            var queryParams = currentDataUrl.split('&');
            // strip out is latest edition filter since we're starting a new query
            queryParams = $.grep(queryParams, function (param, idx) {
                var key = param.split('=')[0];
                return (key.indexOf('fd_IsLatestEdition') < 0);
            });
            currentDataUrl = queryParams.join('&');
            ...
            $('.querybuilder-wrap .querybuilder-instance').each(function (i) {
                ...
                if (instanceVisible) {
                    ...
                    var qbInputElement = $this.find('#instance_QueryBuilderSearchTerm');
                    // If the default behaviour failed, fall back to OUP-specific class-name
                    if (qbInputElement.length === 0) {
                        qbInputElement = $this.find(".querybuilder-input").val().trim();
                    }
                    else {
                        qbInputElement = qbInputElement.val().trim();
                    }
                    ...
                    if (instanceIndex === 1) {
                        ...
                        var searchTerm = encodeURIComponent(qbInputElement);
                        ...
                        if (typeof (qbSearchTypeFieldName) !== "undefined" && qbSearchTypeFieldName !== null && $.trim(qbSearchTypeFieldName) !== "" && searchTerm !== "") {
                            ...
                            if (encodeURIComponent(currentSearchTerm) !== searchTerm) {
                    ...
                    switch (qbSearchTypeDisplayName) {
                        case "":
                            ...
                            if (qbInputElement !== "") {
                                var qbSearchTerm = qbInputElement.replace(/^"(.*)"$/, '$1');
                                currentDataUrl = updateDataUrlForAllOption(qbSearchTerm, currentDataUrl);
                                qbObject[keyOptionOrdinal] = encodeURIComponent(qbSearchTerm);
                                qbObjectInitialized = true;
                            }
                            break;
                        case "All":
                            if (qbInputElement !== "") {
                                var qbSearchTerm = encodeURIComponent(qbInputElement);
                                currentDataUrl = updateDataUrlForAllOption(qbSearchTerm, currentDataUrl);
                        ...
                        default:
                            ...
                            if (qbInputElement !== "") {
                                var qbSearchTerm = qbInputElement.replace(/^"(.*)"$/, '$1');
                                qbObject[keyDefName] = encodeURIComponent(qbSearchTerm);
                                qbObjectInitialized = true;
                            }
                            break;
                    }
        ...
        SCM.QueryBuilder.defaults.runSearch(queryString);
    });
...
SCM.ArticleImageDetails = (function ($) {
    ...
    //Declare this to save and access later to return focus to last active element.
    var lastActive;
    ...
    function _handleOpenAndClose(event) {
        ...
        $("#" + itemNum).toggleClass('active-modal');

        _toggleBodyOverflow();

        _getCitation(itemNum);

        if ($('.active-modal').length) {
            _trapFocus();
        } else {
            lastActive.focus();
        }
    ...
    //Store value for last active element before entering modal.
    $('.js-item-image').on('click', function () {
        lastActive = document.activeElement;
    });

    //This is called to search for the clickable elements within the active modal and prevent focus from escaping while in the modal.
    function _trapFocus() {
        
        var activeModal = document.querySelector('.active-modal .figureModal');
        var focusableEls = activeModal.querySelectorAll('a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])');
    ...
    function _handleModalNavigation(event) {
        ...
        $currentItem.removeClass('active-modal');

        var newItemNum = $this.data("new-item");
        var $newItem = $("#" + newItemNum);
        $newItem.addClass('active-modal');
...
(function (ns, $, $document, undefined) {
    ...
    ns.enableCitationDownloadOnFormatSelect = function() {
    ...
    function contentFilter(widget,dataName) {
        ...
        if ($fullTextContent.length) {
            ...
            var $supplementaryDataContent = $contentSections.has('.widget-ArticleDataSupplements, .supplementary-material');
            ...
            var setDownloadImageLink = function ($contentSections) {
                // Need to do both filter and find to include parents and children
                if ($contentSections.filter('.fig-section:not(:has(a[restricted]))').length || 
                    $contentSections.find('.fig-section:not(:has(a[restricted]))').length) {
            ...
            $filterMenu.on('click', function () {
                ...
                var activeContentFilter = $('.js-article-content-filter[data-content-filter="' + contentFilter + '"]');
                activeContentFilter.hide();
                activeContentFilter.siblings().show();
    ...
    ns.init = function () {
        ...
        ns.enableCitationDownloadOnFormatSelect();
    };
...
SCM.JumpLinks = (function($){
    ...
    */
    return {
        init: init,
        registerNotesTocLink: registerNotesTocLink,
        registerFootNotesLink: registerFootNotesLink
    }

    function init() {
        $body = $('html, body');
        $jumpLinks = $('.js-jumplink, .sectionLink').not('.disable').not('.hide');
        ...
        if ($body.hasClass('pg_article') || $body.hasClass('pg_issue')) {
            ...
            if (window.location.hash) {
                var targetIdValue = window.location.hash.split('#').pop().trim();
                $target = findJumplinkTarget('#' + targetIdValue);
        ...
        $jumpLinks.on('click', function (e) {
            if ($body.hasClass('pg_issue')) {
            ...
            else if ($body.hasClass('pg_article') || $body.hasClass('pg_chapterstandard')) {
                var href = $(this).attr('href');
                $target = findJumplinkTarget(href);

                var targetOffsetTop = $body.hasClass('pg_chapterstandard')
                ...
                $(".page-column--left").removeClass('js-filters-open');
    ...
    */
    ...
    /**
    * register click event for manually added notes link on TOC
    * this is added after chapter display, if end notes widget exists
    */
    function registerNotesTocLink(notesDiv) {
        $('.js-notes-link').on('click', function () {           
            var targetOffsetTop = $(notesDiv).offset().top - 70;
            $body.animate({
                scrollTop: targetOffsetTop
            }, 400, function () {
                $(window).off('scroll', handleScroll);
    ...
    function registerFootNotesLink() {
        $(".js-end-note-link").off('click');  // unbind event when switching between split view and standard view
        $(".js-end-note-link").on('click', function (e) {
            var footnoteId = $(this).attr('data-fn-id');
            var bookId = $("#bookId").val();
            var isArticle = $('.article-body:visible').length;
            if (footnoteId && footnoteId.length) {
                var $noteLink;
                if (isArticle) {
                    $noteLink = $('.content-inner-wrap:visible .js-xref-fn[reveal-id="' + footnoteId + '"]:visible');
                    // special case for table-modals that are marked display:block and visibility:hidden
                    // Have to filter those out and only look at ones not in the modal
                    if ($noteLink.length > 1) {
                        $noteLink = $noteLink.filter(function (idx) {
                            var $parent = $($noteLink[idx]).closest('.table-modal.table-full-width-wrap');
                    ...
                    $noteLink = $('.js-xref-fn[reveal-id="' + footnoteId + '"]:first');
                }
                if ($noteLink.length === 0 && bookId.length) {
                    // If it's on this page don't need 
                    // to find the chapter on the server side
                    // 2 types of links - footnote links and xref links - check both
                    $noteLink = $('.js-related-object[legacysectionid="' + footnoteId + '"]:first');
                }
                if ($noteLink.length) {
                    // need to account for sticky toolbar && splitview
                    // Only want split pane if it's visible
                    var $splitPane = $('.content-inner-wrap:visible').closest('#split-view-scroll-pane');
                    var pageOffset = 0;
                    var $pageBody = $body;
                    var targetOffsetTop = $noteLink.offset().top - 60
                    if ($splitPane.length !== 0) {
                        // have to scroll from a different place in split view
                        pageOffset = $('.content-inner-wrap:visible').offset().top;
                        $pageBody = $('.content-inner-wrap:visible').closest('#split-view-scroll-pane');
                        targetOffsetTop = Math.abs(pageOffset - $noteLink.offset().top);
                    }
                    $pageBody.animate({
                        scrollTop: targetOffsetTop
                    }, 200, function () {
                ...
                // Don't need to do this for articles, since footnotes will be on the current page
                else if (!isArticle && bookId.length) {
                    var baseUrl = App.CurrentSubdomain.length ? '/' + App.CurrentSubdomain : '';
                    $.ajax({
                        url: baseUrl + '/Content/GoToChapterFootNote',
                        cache: false,
                        data: {
                            bookId: bookId,
                        ...
                        //if failure, we couldn't find it so do nothing
                    });
                }
            }
        });
        ...
        $(window).on('scroll', function () {
            ...
            if ($('.book-chapter-body').length) {
                if (!$jsJumpLinkTargetIds.length) {
                    $('#ContentColumn h2, #ContentColumn h3, #ContentColumn h4').each(function () {
                        $jsJumpLinkTargetIds.push($(this));
                    });
                    // if there is a notes link in the TOC, add the footnotes element as one of the jumplinks
                    if ($(".js-toc-footnotes-link").find(".js-notes-link:visible").length > 0) {
                        $jsJumpLinkTargetIds.push($('#chapter-footnotes'));
                    } // if there is a notes link in the TOC, add the endnotestargetlocation element as one of the jumplinks
                    if ($(".js-toc-notes-link").find(".js-notes-link:visible").length > 0) {
                        $jsJumpLinkTargetIds.push($('#endNotesTargetLocation'));
                    }
                }
                ...
                $($jsJumpLinkTargetIds).each(function () {
                    ...
                    if ($(this).data('magellan-destination')) {
                        ...
                        subheadingChapterId = parseInt($(this).prevAll('.jumpLoc-sectionId').eq(0).attr('id'));
                    }

                    // special case for End notes widget link
                    // widget displays when chapter has end notes, we manually add Notes link in TOC to jump to it
                    if ($(this).hasClass('js-end-notes-target') || $(this).hasClass('js-footnotes-target')) {
                        scrollToLink = $('#InfoColumn .js-section-title-wrap .js-notes-link:visible');
                    }
                    else {
                        scrollToLink = $('#InfoColumn .js-section-title-wrap a[data-sectionid="' + subheadingChapterId + '"]');
                    }
                    ...
                    if ((topDistance - 120) < scrollTop) {
                        if ($(scrollToLink).length) {
                            ...
                            $jsJumplinks.removeClass('active');
                            $(scrollToLink).closest('.js-jumplink').addClass('active');
                            $(scrollToLink).parents('.js-headings-placeholder').siblings('.js-section-title-wrap').find('.js-tocLink').addClass('active');
                            $(scrollToLink).parents('.js-headings-placeholder').siblings('.js-section-title-wrap').find('.js-jumplink').addClass('active');
                        }
                    }
                });
            }
...
(function (ns, $) {
    ...
    ns.init = function () {
        ...
        $stickyElements = $(".js-left-nav-col, .js-toolbar-wrap, .sr-filters, .js-mobile-nav");
        ...
        var $leftNavStickyOffsetHiddenField = $('#hfLeftNavStickyOffset');
        if ($leftNavStickyOffsetHiddenField.length) {
            leftNavStickyOffset = parseInt($leftNavStickyOffsetHiddenField.val());
        }
    ...
    */
    ...
    function isPageArticle() { return $body.hasClass('pg_article'); }
    function isPageArticlesByGroup() { return $body.hasClass('pg_articlesbygroup'); }
    function isPageIssue() { return $body.hasClass('pg_issue'); }
    function isPageSearch() { return $body.hasClass('pg_searchresults'); }
    function isPageSelfServe() { return $body.hasClass('pg_selfserve/sspage') || $body.hasClass('pg_selfservepage'); }
    function isPageBook() { return $body.hasClass('pg_book'); }
    function isPageChapter() { return $body.hasClass('pg_chapterstandard'); }
    ...
    function cloneToolbarToStickyToolbar() {
        if (!isToolbarCloned()) {
            var $clone = $toolbarWrap.clone(true);
    ...
    function updateLeftNav() {
        ...
        if (lastScrollPosY && $header && lastScrollPosY >= $header.height()
                            + $header.offset().top
                            + guardianAdHeight
                            - leftNavStickyOffset) {
...
(function (ns, $) {
    ...
    ns.setAccessIcons = function (accessIcons) {
        accessIcons = accessIcons || SCM.AccessIcons;

        var currentResourceList = $('[data-resource-id-access]');
        if (currentResourceList.length && accessIcons) {
            currentResourceList.each(function (index, item) {
                var $this = $(this);
                var resourceId = $this.data('resourceIdAccess');

                var accessIcon = SCM.SiteJS.getAccessIcon(resourceId, accessIcons);
                if (accessIcon) {
                    var $placeholder = $this.find('.js-access-icon-placeholder');

                    // replace icon css class
                    $placeholder.removeClass(accessIcon.icon).addClass(accessIcon.icon);

                    // replace icon title
                    $placeholder.attr('title', accessIcon.title);                    
                }
            });
        }
    };

    ns.getAccessIcon = function (resourceId, accessIcons) {
        accessIcons = accessIcons || SCM.AccessIcons;

        var accessIcon;
        accessIcons.map(function (x, y) { if (x.id === resourceId) { accessIcon = x; return false; } });
        return accessIcon;
    };
...
(function (ns, $) {
	...
	ns.registerAnswerLinkClick = function () {
		$('.js-answer-link').on('click', function () {
			var $this = $(this);
			$this.next('.js-answer-wrap').toggleClass('hide');
			$this.find('.js-answer-show').toggleClass('hide');
			$this.find('.js-answer-hide').toggleClass('hide');
		});
	};
...
// WARNING: This module must be "widget instance safe". All selectors must be relative to the clicked node.
// This is because there can be multiple instances of the widget.
(function (ns, $) {

    var maxLevel = $('#hfMaxRelativeDepth').length ? parseInt($('#hfMaxRelativeDepth').val()) : 0;
    var browseContentInLinkText = $('#hfBrowseContentInLinkText').val();
    var _transitionLength = 100;

    function _nodeGreaterThanMaxLevel() {
        var $this = $(this);    // Item in the list of all nodes
        return parseInt($this.attr('data-level')) > maxLevel;
    }

    function goDeeper() {
        var $this = $(this);
        var $currentNode = $this.closest('.js-tree-node-item');
        var $context = $currentNode.closest('.js-tree-browse-wrap');
        var currentLevel = $currentNode.attr('data-level');
        var newRootNode = $currentNode.attr('data-id');
        var $additionalNodes = null;

        // If we're at the max level, show any nodes that are deeper.
        if (currentLevel === maxLevel) {
            $additionalNodes = $currentNode.next('.js-children').find('.js-tree-node-item').filter(_nodeGreaterThanMaxLevel);
        }

        changeLevelOfNodesDisplayed($context, newRootNode, $additionalNodes);

        setTimeout(function () {
            updateBreadcrumbsToGoDeeper($context, $currentNode, newRootNode);
        }, _transitionLength);

        updateBrowseLink($context, $currentNode);
    }

    /// Animation Step (Start)
    function changeLevelOfNodesDisplayed($context, newRootNode, $additionalNodes) {
        // Set to show only those specified
        var $nodesToShow = $context.find('.js-tree-node-item[data-parent-id="' + newRootNode + '"]');
        if ($additionalNodes && $additionalNodes.length) {
            $nodesToShow = $nodesToShow.add($additionalNodes);
        }

        // Reset all nodes
        var $allNodes = $context.find('.js-tree-node-item');
        $allNodes.removeClass('fade');
        var originalTotalHeight = $context.find('.js-tree-browse-nodes-wrap').height();
        $context.find('.js-tree-browse-nodes-wrap').height(originalTotalHeight);

        hideAllNodes($allNodes)
            .then(function(originalHeight) {
                return activateNewNodes($context, $nodesToShow, originalHeight);
            })
            .then(function() {
                return fadeInNodes($nodesToShow);
            });
    }

    /// Animation Step
    function hideAllNodes($allNodes) {
    ...
    /// Animation Step
    function updateBreadcrumbsToGoDeeper($context, $currentNode, newRootNode) {
        return new Promise(function(resolve, reject) {
            // You clicked on a node with an arrow. Add it to the breadcrumbs.
            var currentNodeLabel = $currentNode.find('.js-tree-node-link').text();
            var $breadcrumbs = $context.find('.js-tree-browse-breadcrumb-link');

            // Reveal the breadcrumbs if we haven't already
            var $breadcrumbWrapWrap = $breadcrumbs.closest('.js-tree-browse-breadcrumb-wrap');
            var $breadcrumbWrap = $breadcrumbs.closest('.js-breadcrumbs');

            if (!$breadcrumbWrap.is('.active')) {
                $breadcrumbWrapWrap.height(0);
            }
            
            $breadcrumbWrap.addClass('active');
            $breadcrumbWrap.addClass('fade');

            setTimeout(function () {
                $breadcrumbWrapWrap.height($breadcrumbs.closest('.js-breadcrumbs').outerHeight());
            }, _transitionLength);

            $breadcrumbs.removeClass('active');
            var $nextEmptyBreadcrumb = $breadcrumbs.siblings('.hide').first();
            $nextEmptyBreadcrumb.text(currentNodeLabel);
            $nextEmptyBreadcrumb.attr('data-id', newRootNode);
            $nextEmptyBreadcrumb.removeClass('hide');
            $nextEmptyBreadcrumb.addClass('active');
            resolve();
        });
    }

    /// Animation Step
    function activateNewNodes($context, $nodesToShow) {
        return new Promise(function(resolve, reject) {
            setTimeout(function() {
                $nodesToShow.addClass('active');
                var newNodesHeight = $context.find('.js-tree-browse-nodes').outerHeight();
                $context.find('.js-tree-browse-nodes-wrap').height(newNodesHeight);
                resolve();
            },
            _transitionLength);
        });
    ...
    /// Animation Step
    function fadeInNodes($nodesToShow) {
        return new Promise(function(resolve, reject) {
            setTimeout(function() {
                $nodesToShow.addClass('fade');
                resolve();
            },
            _transitionLength);
        });
    ...
    /// Animation Step
    function updateBrowseLink($context, $currentNode) {
        var currentNodeLabel = $currentNode.find('.js-tree-node-link').text();
        var newRootNode = $currentNode.attr('data-id');

        return new Promise(function(resolve, reject) {
            setTimeout(function() {
                $context.find('.js-tree-browse-link-all').addClass('hide');
                var $narrowLink = $context.find('.js-tree-browse-link-narrowed');
                $narrowLink.text(browseContentInLinkText + currentNodeLabel);
                $narrowLink.attr('href', $narrowLink.data('href-template').concat(newRootNode));
                $narrowLink.removeClass('hide');
                resolve();
            },
            _transitionLength);
        });
    ...
    function breadcrumbClick() {
        var $this = $(this);
        var $context = $this.closest('.js-tree-browse-wrap');
        $this.nextAll().addClass('hide');
        $this.addClass('active');
        var newId = $this.attr('data-id');
        changeLevelOfNodesDisplayed($context, newId);

        // If you clicked the root breadcrumb, hide all breadcrumbs.
        if ($this.is('.js-root-breadcrumb')) {
            $breadcrumbsWrap = $this.parents('.js-breadcrumbs');
            ...
            // Change the "Browse content in ..." link to "Browse all content"
            $context.find('.js-tree-browse-link-all').removeClass('hide');
            $context.find('.js-tree-browse-link-narrowed').addClass('hide');
        } else {
            var currentNodeLabel = $context
                .find('.js-tree-node-item[data-id="' + newId + '"]')
                .find('.js-tree-node-link')
                .text();
            var $narrowLink = $context.find('.js-tree-browse-link-narrowed');
            $narrowLink.text(browseContentInLinkText + currentNodeLabel);
            $narrowLink.attr('href', $narrowLink.data('href-template').concat(newId));
            $narrowLink.removeClass('hide');
        }
    }

    ns.checkPageHeightForSeriesDropdown = function () {
        var $contentPanel = $('.js-main');
        if ($("#searchResultsPage").length !== 0) {
            $contentPanel = $("#ContentColumn");
        }
        if ($contentPanel.length > 0) {
            var contentHeight = $contentPanel.height();
            var maxHeight = contentHeight;
            var $treeDropdowns = $('.js-tree-browse-wrap');
            $treeDropdowns.each(function (idx) {
            ...
            if (maxHeight > contentHeight) {
                $contentPanel.height(maxHeight);
            }
        }
    }

    // Called by the main init in client.script.js. This handles all instances. The other functions must be "widget instance safe".
    ns.init = function () {
        $(document.body).on('click', '.js-tree-node-link', goDeeper);
        $(document.body).on('click', '.js-tree-browse-breadcrumb-link', breadcrumbClick);
        ns.checkPageHeightForSeriesDropdown();
    };
...
(function (ns, $) {
    ...
    var $body = $('body'),
        ...
        $navbarSearchContainer = $(".js-navbar-search-container"),
        ...
        $navbarSearchInput = $('.navbar-search .navbar-search-input');
    ...
    ns.isPageViewLarge = function () {
        if ($body.hasClass("pg_ViewLarge")) {
            return true;
        }
    }
    ...
    ns.showAuthorInfo = function () {
        var matchList = ".js-linked-name-trigger, .linked-name, .al-author-name .linked-name, .al-author-name-more .linked-name, .linked-name-byline, .al-author-footnotes .js-linked-footnotes";
   

        $(matchList).on('click', function (e) {
            var $targetModal = $(this).siblings('.al-author-info-wrap');
            e.stopPropagation();
            $('.al-author-info-wrap').not($targetModal).removeClass('open');
            $($targetModal).toggleClass('open');
            $($targetModal).attr('tabindex', '-1').focus();
    ...
    // Just for Books, look for anchor objects containing ISBN, these are unresolved links)   
    // See NARN-14328. Do not route to EndMatter Page for related objects with end-notes. See NARN-11737
    ns.handleUnresolvedRelatedObjectLinks = function () {
        $(document).on('click', 'a[isbn][object-type!="end-note"], .js-link-page', function () {
            // default redirect to 404 page
            var redirectUrl = "/CustomError?statusCode=404";

            // get link properties
            var link = $(this);
            var isbn = link.attr("isbn");
            var bookId = $("#bookId").val();
            var legacySectionId = link.attr("legacySectionId");
            var $relatedObj = $("div[name='" + legacySectionId + "']");
            // If it's on this page, don't need to find the related objecct on the server side
            if ($relatedObj.length) {
                window.location.hash = legacySectionId;
                return;
            }
            isbn = isbn === null || isbn === undefined ? "" : isbn.trim();
            legacySectionId = legacySectionId === null || legacySectionId === undefined ? "" : legacySectionId.trim();

                // Is there an ISBN we can use?
            if (isbn !== "" || legacySectionId !== "") {

                var baseUrl = App.CurrentSubdomain.length ? '/' + App.CurrentSubdomain : '';
                $.ajax({
                    url: baseUrl + '/Book/GoToRelatedObjectLink',
                    cache: true,
                    dataType: 'json',
                    data: {
                        isbn: isbn,
                ...
                .done(function (response) {
                    if (response && response.Success) {
                        window.top.location.assign(response.RedirectURL);
                    }
                    else {
                        // if it failed, go to 404 page
                        console.log("/Book/GoToRelatedObjectLinkfailed!");
    ...
    ns.launchGetCitationModal = function () {
        $(document.body).on('click', '[data-reveal-id="getCitation"]', function (e) {
            $('#getCitation').foundation('reveal-modal', 'open');
        });
        $(document.body).on('opened.fndtn.reveal', '#getCitation', function () {
        ...
        $(document.body).on('closed.fndtn.reveal', '#getCitation', function () {
            $('.js-cite-button').focus();
        });
    };

    ns.launchSupersededContentModal = function () {
        $('#supersededContent').on('opened.fndtn.reveal', function () {
            $('.js-latest-link').focus();
        });
        $('#supersededContent').on('closed.fndtn.reveal', function () {
            $(document.body).focus();
        });
        $('#supersededContent').foundation('reveal', 'open');
    };
    ...
    ns.showFullVersionInfo = function () {
        $('.js-version-info-more-link').on('click', function () {
    ...
    **********************************************************************/
    ns.closeNavbarSearch = function () {
        $closeNavSearch.on('click', function () {
            $navbarSearchContainer.removeClass('tablet-viewport-active');
            $navbarSearchElements.hide();
            $navbarMenu.fadeIn();
        });
    };
    ...
    ns.expandNavbarSearch = function () {
        $expandNavSearch.on('click', function () {
            $navbarSearchContainer.addClass('tablet-viewport-active');
            $navbarMenu.hide();
            $navbarSearchElements.fadeIn(); // fade in
            $navbarSearchInput.focus(); // move focus to the input
        });
    };


    ns.toggleSearchCover = function () {
        if ($('.sf-facet, .js-taxonomy-label').length) {
            $('.sf-facet, .js-taxonomy-label').on('click', function () {
                $(document).ajaxStart(function () {
                    $('.info-inner-cover').removeClass("uncovered");
                    $('.info-inner-cover').addClass("covered");
                });
    ...
    ns.toggleMobileSiteMenuSubnav = function () {
        $('.dropdown-menu .site-menu .site-menu-item').on('click', function () {
            var $this = $(this),
                $thisArrow = $this.children('.mobile-nav-arrow');
            $this.children('.site-menu').slideToggle();
    ...
    ns.openRevealModal = function ($obj) {
        ...
        //save last focus to return after modal closes
        var lastFocusedElement = document.activeElement;
        //change focus to modal that is receiving class of open
    ...
    ns.revealFigsTablesInModal = function () {
        ...
        $body.on('click', modalTriggerElems, function () {
            ...
            var revealId = $this.attr('reveal-id') ? $this.attr('reveal-id') :
                ($this.attr('data-reveal-id') ? $this.attr('data-reveal-id') : $this.attr('data-id'));
    ...
    ns.manageMobileDropdowns = function (thisObject, dropdownClass) {
        ...
        if ($(".dropdown-panel").hasClass('is-active')) {
            ...
            $mobileSearchInput.focus(); //give focus to the input
        }
    ...
    ns.toggleMobileDropdownSubnav = function () {
        ...
        $navLink.on('click', function (e) {
            ...
            if ($this.siblings('.site-menu-lvl-1').length) {
                $this.siblings('.site-menu-lvl-1').slideToggle();
            } else {
                $this.siblings().find('.site-menu-lvl-1').slideToggle();
            }
            $targetArrow.toggleClass('icon-general_arrow-down');
            $targetArrow.toggleClass('icon-general_arrow-up');
        });
    };
    ...
    ns.Login = function ($formContainer, $formSpinner) {
        ...
        if (honeypotValue.length > 0 && console.warn) {
        ...
        var simpleCaptchaEnabled = $('.js-captcha-simple').length > 0;
        var normalCaptchaEnabled = $formContainer.find('.js-captcha-normal').length > 0;
    ...
    ns.alertKeyPress = function () {
        $('.js-user-alert').on('keydown', function (event) {
            if (event.key === 'Enter') {
                //The triggerHandler( "event" ) method will not call .event() on the element it is triggered on. This is to 
                //avoid conflict with the actual click being detected in ns.alertLinks.
                //This is an odd use case of the user tabbing through the entire issue page directly to the alerts and pressing enter or return.
                //Because ns.alertLinks is using each for a common class, rather than using .trigger, .triggerHandler() only affects the first matched element.
    ...
    ns.alertLinks = function () {
        $('.js-user-alert').each(function () {
            var userAlert = $(this);
            userAlert.on('click', function () {
                //Remember element clicked to return focus after closing modal in function showSignUpAlertInfo.
                activeElement = userAlert;
                ...
                var siteid = userAlert.data('siteid');
                if (isUserLoggedIn !== undefined && isUserLoggedIn.toLowerCase() === 'false') {
                    ...
                    if ((siteid === 191 || siteid === 192) && alerttype === 3) {
                        window.location = '/my-account/email-alerts';
                    }
                    else {
    ...
    ns.showSignUpAlertInfo = function (userAlert) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $(document).on('opened.fndtn.reveal', '[data-reveal]', function () {
                    var activeModal = document.querySelector('.open');

                    //Trap focus within modal here.
                    var focusableEls = activeModal.querySelectorAll('a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled]), input');
                    var firstFocusableEl = focusableEls[0];
                    var lastFocusableEl = focusableEls[focusableEls.length - 1];
                    var KEYCODE_TAB = 9;
                    firstFocusableEl.focus();
                ...
                var scheduleSelect = $('.userAlertSignUp').find("#sezlectAlertFreqeuncy");
    ...
    ns.taxonomyFilterFunction = function () {
        return $(this).parents('.js-taxonomy-child-list-item').parents('.js-taxonomy-child-list-item').children('.js-taxonomy-parent-label').find('.js-taxonomy-checkbox:checked').length < 1;
    };

    ns.initTaxonomyArrows = function (parentWindow) {
        parentWindow.find('.js-taxonomy-parent-arrow').on('click', function () {
            var $this = $(this);

            $this.parents('.js-taxonomy-label-wrap').siblings('.js-taxonomy-child-list').toggleClass('hide');

            var $icon = $this.children('.js-taxonomy-icon');
            $icon.hasClass('icon-general-arrow-filled-right')
                ? $icon.removeClass('icon-general-arrow-filled-right').addClass('icon-general-arrow-filled-down')
                : $icon.removeClass('icon-general-arrow-filled-down').addClass('icon-general-arrow-filled-right');
        });
        ...
        parentWindow.find('.js-taxonomy-checkbox').on('click', function () {
            ...
            if ($(this).is(":checked")) {
                ...
                $(this).closest('.js-taxonomy-node').children('.js-taxonomy-parent-label').find('.js-taxonomy-checkbox').prop('checked', false);

                // uncheck parent nodes that aren't the toplevel node
                $(this).parents('.js-taxonomy-child-list-item').children('.js-taxonomy-parent-label').find('.js-taxonomy-checkbox').prop('checked', false);
            }
        });
    };
    ...
    ns.initSearchResultsSort = function () {
        ...
        // Adding .off to avoid multiple bindings here - this function 
        // is called after on an AjaxComplete event
        // SolrSearch is done with ajax.  Don't want to avoid calling it all together
        // in case it's needed for something else
        $sortOrderSelect.off('change').on('change', function () {
            var qs = $(this).val();
            SCM.SearchResults.doSolrSearch({ queryString: qs, source: "searchresults" });
        }); // END sortOrderSelect.on('change')

        $sortOrderSelectOption.off('click').on('click', function () {
            ...
            linkNewAndPopularMode.on('click',
                function (e) {
                    ...
                    if (!articleListNewPopContent.hasClass('hasContent')) {
                        var widgetInstanceName = getWidgetInstanceName('.widget-ArticleListNewAndPopular');
                        SCM.SiteJS.getArticleListNewAndPopularContent(mode, articleListNewPopContent, widgetInstanceName);
    ...
    function getWidgetInstanceName(widgetClass) {
        var widgetInstanceName;
        var className = $(widgetClass).attr('class');
        if (className.indexOf("widget-instance-") >= 0) {
            widgetInstanceName = className.substr(className.indexOf("widget-instance-")).split("-")[2];
        }
        return widgetInstanceName;
    }

    ns.getArticleListNewAndPopularContent = function (mode, articleListNewPopContent, widgetName) {
        $.ajax({
            url: (App.CurrentSubdomain.length ? '/' + App.CurrentSubdomain : '') + '/PlatformArticle/ArticleListNewAndPopularContent/' + (widgetName.length ? '/' + widgetName : ''),
            cache: false,
            data: {
                mode: mode
            },
    ...
    ns.toggleSearchResultsDetails = function () {
        ...
        $switchContainer.off('click').on('click', function (e) {
            e.stopPropagation();
    ...
    //icon arrow aria expanded toggle
    ns.citationAriaExpand = function () {
        var ariaExpandToggle = $('.js-journal-citation-filter-toggle');
        var dropdown = $('.journal-citation-filter-input-wrap');
        ariaExpandToggle.attr("aria-expanded", "true");
        ariaExpandToggle.click(function () {
            if (dropdown.is(".collapsed")) {
                ariaExpandToggle.attr("aria-expanded", "true");
            } else {
                ariaExpandToggle.attr("aria-expanded", "false");
            }            
        });
    };
    ...
    ns.abstractMathJax = function () {
        if ($('.js-abstract-response-placeholder').length) {
            MathJax.Hub.Queue(["Typeset", MathJax.Hub, ".js-abstract-response-placeholder"]);
        }
    }
    ...
    ns.init = function () {
        ...
        ns.showFullVersionInfo();
        ns.handleUnresolvedRelatedObjectLinks();
    ...
    ns.windowLoadInit = function () {
        ...
        ns.launchSupersededContentModal();
...
(function ($, undefined) {
    ...
    $(window).on("resize", function (e) {
        ...
        $resizeTimer = setTimeout(function () {
            ...
            if (viewportWidth !== newWidth) {
                ...
                if (viewportWidth <= 930 || viewportWidth >= 1101) {
                    ...
                    //This is related to functions above: closeNavbarSearch and expandNavbarSearch.
                    $('.js-navbar-search-container').removeClass('tablet-viewport-active').addClass('tablet-viewport-layout');
                }
                ...
                if (viewportWidth >= 931) {
                    ...
                    $(".dropdown-panel-wrap").children(".is-active").hide().removeClass('is-active');
                    $(".mobile-dropdown-toggle").removeClass('is-active');
                }
        ...
        // check for height of subject dropdown
    ...
    $(document).ajaxStart(function () {
        ...
        // Need to do this to prevent recursion...
        if (settings.url.indexOf('brightcove') < 0) {
            SCM.Utilities.Brightcove.init();
        }
        SCM.TreeBrowse.checkPageHeightForSeriesDropdown();
    });
...
jQuery(document).ready(function ($) {
    ...
    if (window.location.hash) {
        $('.js-jumplink[href="' + window.location.hash + '"]')
            .addClass('active')
...
(function (ns, $) {
            ...
            $.post('/OUPMyAccount/UpdateAccount',
                ...
                function (data) {
                    if (data != null) {
                        ...
                        if (data.Success) {
                            $('.js-header-account-info-user-fullname').html(firstName + ' ' + lastName);
    ...
    ns.initRegister = function (lastError) {
        ...
        $formContainer.on('click', '#registerBtn', function () {
            ...
            var simpleCaptchaEnabled = $('.js-captcha-simple').length;
            var normalCaptchaEnabled = $('.js-captcha-normal').length;
            var txtCaptchaInputId = $('.js-captcha-input').val();
            ...
            if (honeypotValue.length > 0) {
                ...
                $.post('/OUPMyAccount/Register',
                {
                    ...
                    captchaResponse: normalCaptchaEnabled ? grecaptcha.getResponse() : '',
                    simpleCaptchaResponse: simpleCaptchaEnabled ? txtCaptchaInputId : ''
                },
                function (result) {
                    if (result === 'OK') {
...
(function (ns, $, bc) {
    ...
    function handleClickEvent() {
        ...
        var isLayAbstract = $this.data('is-lay-abstract') === 'True';
        var $abstractLocation = getAbstractLocation(articleId, isLayAbstract);
        var $abstractLinks = $('#abstract-' + articleId + '-links');
        var ajaxUrl = getAjaxUrl($this, isLayAbstract);
        
        $abstractLocation.closest('.js-abstract-parent').toggleClass('abstractExpanded');

        var isAbstractAlreadyFetchedFromServer = $this.data('tried-to-fetch-abstract') === 'True';
        if (!isAbstractAlreadyFetchedFromServer) {
            toggleSpinner($abstractLocation);

            $.get(ajaxUrl, { articleId: articleId, layAbstract: isLayAbstract })
                .done(function (data) {
        ...
        // Toggle Abstract Icon
        var $thisAbstractIcon = $this.find('.js-abstract-toggle-icon');
        var toggleIconClass = $thisAbstractIcon.data('icon-class');
        var downArrow = toggleIconClass + '-down';
        var upArrow = toggleIconClass + '-up';

        $thisAbstractIcon.toggleClass(downArrow);
        $thisAbstractIcon.toggleClass(upArrow);
    }

    function getAbstractLocation(articleId, isLayAbstract) {
        if (isLayAbstract) {
            return $('.js-layabstract#abstract-' + articleId);
        } else {
            return $('#abstract-' + articleId);
        }
    }

    function getAjaxUrl($linkContext, isLayAbstract) {
        var ajaxUrl = '';

        var baseSiteUrl = getBaseSiteUrl();
        if (baseSiteUrl) {
            ajaxUrl += '//' + baseSiteUrl;
        }

        if (isLayAbstract) {
            ajaxUrl += '/PlatformArticle/ArticleLayAbstractAjax';
        } else if ($linkContext.data('abstract-type') === 'extract') {
            ajaxUrl += '/PlatformArticle/ArticleAbstractOrExtractAjax';
        } else {
            ajaxUrl += '/PlatformArticle/ArticleAbstractAjax';
        }
    ...
    function handleClickEventForTabs() {
        ...
        $(document).on('click', '.js-abstract-tab-link', function(e) {
            var $target = $(e.target);
            var $context = $target.closest('.js-abstract-response-placeholder');
            var $parent = $target.closest('.js-abstract-tab-link-wrap');
            var $content = $context.find('.js-tabs-content .js-abstract-tab-content-wrap');
            var tabNum = $parent.index();

            switchTabLinks($context, $parent);
            switchTabContent($content, tabNum);
        });
    }

    function switchTabLinks($context, $clickedLink) {
        $context.find('.js-abstract-tab-link-wrap').removeClass('is-active');
        $clickedLink.addClass('is-active');
    }

    function switchTabContent($content, tabNum) {
        $content.removeClass('is-active');
        $content.eq(tabNum).addClass('is-active');
    }

    function toggleSpinner($abstractLocation) {
        $abstractLocation.find('.js-spinner').toggleClass('hide');
    }
...
$(function () {
    ...
    $(document).on('click', '.facetShowMore', function (event) {
        var $facetList;
        // if filter has child nodes (Series taxonomy filter), then need to hide/show top level nodes
        // while retaining user's manual expansions
        var hasChildren = $(this).parent().hasClass('sf-tax-facetShowMoreLi');
        if (hasChildren) {
            $facetList = $('.sf-tax-facet-list.taxonomy-lvl-0').children('.sf-tax-facet:gt(11):visible');
        }
        else {
            $facetList = $(this).closest('.sf-facet-list').find('.sf-facet:gt(11):visible');
        }
        if ($facetList.length) {
            $facetList.animate({ height: 'toggle' }, 400);
            $(this).text('Show more');
        }
        else {
            if (hasChildren) {
                $('.sf-tax-facet-list.taxonomy-lvl-0').children('.sf-tax-facet').not(':visible').animate({ height: 'toggle' }, 400);
            }
            else {
                $(this).closest('.sf-facet-list').find('.sf-facet:gt(11)').not(':visible').animate({ height: 'toggle' }, 400);
            }
            $(this).text('Show less');
        }
        event.preventDefault();
    ...
    function collapseFacets() {
        $('.collapsible-facet .sf-facet-list').each(function () {
            if ($(this).find('.sf-facet').length > 12) {
                $(this).children('.facetShowMoreLi').remove();
                $(this).find('.sf-facet:gt(11)').hide();
                if ($(this).find('.facetShowMoreLi').length === 0) {
                    $(this).append('<li class="facetShowMoreLi"><a href="javascript:;" class="facetShowMore">Show more</a></li>');
                }
            }
        });
        // Series taxonomy filter has child nodes, so here we retain 12 top-level nodes
        $('.sf-tax-facet-list.taxonomy-lvl-0').each(function () {
            if ($(this).children('.sf-tax-facet').length > 12) {
                $(this).children('.facetShowMoreLi').remove();
                $(this).children('.sf-tax-facet:gt(11)').hide();
                if ($(this).find('.sf-tax-facetShowMoreLi').length === 0) {
                    $(this).append('<li class="sf-tax-facetShowMoreLi"><a href="javascript:;" class="facetShowMore">Show more</a></li>');
                }
            }
        });
    }
    ...
    function toggleTaxonomyChildren() {
        ...
        if ($('.js-taxonomy-child-list .js-queryitem').length > 0) {
            $('.js-taxonomy-child-list .js-queryitem').parents('.js-taxonomy-child-list .js-taxonomy-child-list-item').each(function () {
                var $selectedElement = $(this).find('.js-queryitem');
                if ($selectedElement.length > 0) {
                    // expand this child list
                    var $arrowLink = $(this).find('.js-taxonomy-parent-arrow').first();
                    var $icon = $arrowLink.children('.js-taxonomy-icon');
                    if ($arrowLink && $arrowLink.length && $icon.hasClass('icon-general-arrow-filled-right')) {
                        $arrowLink.parents('.js-taxonomy-label-wrap').siblings('.js-taxonomy-child-list').toggleClass('hide');
                        $icon.removeClass('icon-general-arrow-filled-right').addClass('icon-general-arrow-filled-down');
                    }
                }
            });
            $('.js-taxonomy-child-list').each(function () {
                if ($(this).find('.js-taxonomy-checkbox:checked').not(':visible').length > 0) {
                    $(this).find('.facetShowMore').click();
                }
            })
    ...
    (function() {
        ...
        $updateTarget.on('click', '.js-edition-select', function (e) {
            var $inputEl = $(this).siblings('input');
            if ($inputEl.length) {
                var query = $inputEl.data('redirectUrl');
                if (query !== undefined) {
                    SCM.SearchResults.doSolrSearch({ queryString: query });
                }
            }
        });
        ...
        $updateTarget.on('click', '.js-close-button', function(e) {
            var url = $(this).attr('data-url');
            if (url && url !== '#' && url !== 'javascript:;') {
                e.preventDefault();
        ...
        $updateTarget.on('click', '#publicationDate', function () {
            var selected = $(this).is(":checked") ? true : false;
            // if publicationdate is checked, check the hidden electronicpub date too, so we 
            // can add to the filter 
            $("#publicationDateElectronic").prop("checked", selected);
        });
        ...
        $updateTarget.on('click', '#btnRangeSearch', function(e) {
            ...
            // Record all the date types we can select so we can clear the previous query params below
            var dateTypesToRemoveFromPrevQuery = $('.js-date-type-chk').map(function () {
                return $(this).val();
            });
            // find out what's checked
            // we have a hidden one because Published also assumes PublishedElectronic, so have to add both
            // PublishedElectronic is a hidden field that is automatically checked when published is checked
            var $filtersChecked = $('.js-date-type-chk:checked, .js-date-type-chk:hidden:checked');
            ...
            var dateValuesString = SCM.PublicationDateFilter.getSolrDateString(fromYear, toYear, fromMonth, toMonth);
            var dateFilterTypeString = '&dateFilterType=' + (SCM.PublicationDateFilter.isSingleDateSelected ? 'single' : 'range');
           
            var dateString;

            // If they only select the Updated date filter, remove articleDate from the query 
            // since this date (versionDate) doesn't apply to articles  (NARN-12098)
            // and don't add the date values for articleDate to the query string
            if ($filtersChecked.length === 1 && $filtersChecked.val() === 'rg_VersionDate') {
                dateTypesToRemoveFromPrevQuery.push('rg_ArticleDate');
                dateString = dateFilterTypeString + '&' + $filtersChecked.val() + '=' + dateValuesString;
            }
            else {
                // keeping articleDate, so add the dateValues to the beginning which is added to articleDate later on
                dateString = dateValuesString + dateFilterTypeString;
                var $filtersToUse = $filtersChecked;
                // if none are checked, use all filters (NARN-12098)
                if ($filtersChecked.length === 0) {
                    $filtersToUse = $('.js-date-type-chk');
                    // Adding this if no date types are selected so we know what to check/uncheck on return
                    // Behavior and query string is the same for both check all and check none
                    dateString += '&noDateTypes=true';
                }
               
                // Now build the rest of the query string based on the filters
                $filtersToUse.each(function () {
                    dateString += '&' + $(this).val() + '=' + dateValuesString;
                });
            }
            
            // get all the previous query params
            var queryParams = $(this).attr('data-url').split('&');
            // strip out date filter type and date types to filter to replace with new date string built above
            queryParams = $.grep(queryParams, function (param, idx) {
                var key = param.split('=')[0];
                // only return those params which are not dateFilterType and are not the date types
                return (key.indexOf('dateFilterType') < 0 && key.indexOf('noDateTypes') < 0 && $.inArray(key, dateTypesToRemoveFromPrevQuery) < 0);
            });
            // now join the remaining ones back together
            var searchUrl = queryParams.join('&');
            // if we still have articleDate=queryText, replace queryText with the dateString
            // which adds in the date range for articleDate plus datefiltertype and date types
            if (searchUrl.indexOf('queryText') >= 0) {
                searchUrl = searchUrl.replace('queryText', dateString);
            }
            else {
                // otherwise, just add on datefiltertype and date types (versionDate only)
                searchUrl += dateString;
            }
    ...
    SCM.SearchResults = $.extend(SCM.SearchResults, function ($, undefined) {
        ...
        var doSolrSearch = function (myobject) {
            ...
            // Default back to Page 1 when the search has changed
            var page = $.deparam.querystring()["page"];
            if (page && parseInt(page) !== 1) {
                myobject.queryString = myobject.queryString.replace("page=" + page, "page=1");
            }
...
(function (SolrSearch, $, undefined) {
    ...
    var bodyVar = $('body'),
        ...
        hfCurrentBookSearch = $('#hfCurrentBookSearch').val(),
        hfCurrentBookScope = $('#hfCurrentBookScope').val(),
        hfBookSiteScope = $('#hfBookSiteScope').val(),
        hfSeriesScope = $('#hfSeriesScope').val(),
        hfUmbrellaScope = $('#hfUmbrellaScope').val(),
        hfUmbrellaSiteUrl = $('#hfUmbrellaSiteUrl').val(),
        hfUmbrellaSiteId = $('#hfUmbrellaSiteId').val(),
        searchAcrossOA = "",
        queryText = "";
    ...
    $('#navbar-search-filter, #mobile-navbar-search-filter').change(function () {
        ...
        searchAcrossOA = (selected.closest('optgroup').attr('class'));
    ...
    function setAdvancedSearchLinkToCurrentSearchScope() /* void */ {
        if (currentSearchScope === hfparentSiteScope) {
            // Parent site
            $('.js-advanced-search').attr('href', getBaseUrl(hfParentSiteURL) + '/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl }));
        } else if (currentSearchScope === hfBookSiteScope) {
            // Books site
            $('.js-advanced-search').attr('href', getBaseUrl() + '/books/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl }));
        } else if (currentSearchScope === hfjournalSiteScope) {
            // Journals site
            $('.js-advanced-search').attr('href', getBaseUrl() + '/journals/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, passAllJournals: true }));
        } else if (currentSearchScope === hfCurrentBookScope) {
            // Curremt book
            $('.js-advanced-search').attr('href', getBaseUrl(siteUrl) + '/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, searchSiteId: hfparentSiteID, currentBookSearch: true }));
        } else if (currentSearchScope === hfUmbrellaScope) {
            // Umbrella
            $('.js-advanced-search').attr('href', getBaseUrl(hfUmbrellaSiteUrl) + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, searchSiteId: hfUmbrellaSiteId }));
        } else if (currentSearchScope.startsWith(hfSeriesScope)) {
            // Series Taxonomy search
            $('.js-advanced-search').attr('href', getBaseUrl(siteUrl) + '/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, searchSiteId: searchSiteID, series: currentSearchScope }));
        } else if (searchAcrossOA) {
            // Oxford Taxonomy umbrella search
            $('.js-advanced-search').attr('href', getBaseUrl(hfUmbrellaSiteUrl) + '/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, searchSiteId: searchSiteID, series: currentSearchScope }));
        } else if (currentSearchScope !== '') {
            // Taxonomy search
            $('.js-advanced-search').attr('href', getBaseUrl(siteUrl) + '/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, searchSiteId: searchSiteID, taxonomy: currentSearchScope }));
        } else {
            // Current site
            $('.js-advanced-search').attr('href', getBaseUrl(siteUrl) + '/' + getSearchUrl({ baseUrl: defaultAdvancedSearchUrl, searchSiteId : searchSiteID }));
        }
    }
    ...
    var doKeywordsSearch = function (keyword) {
        ...
        var defaultQueryBuilder = 'page=1&qb={Keywords1:' 
            + encodeURIComponent(keyword)
            + '}&SearchSourceType=1';

        var customQueryBuilder = 'cqb=[{%22terms%22:[{%22filter%22:%22Keywords%22,%22input%22:'
            + encodeURIComponent(keyword)
            + '}]}]&qb={%22Keywords1%22:'
            + encodeURIComponent(keyword)
            + '}&page=1&searchType=advanced&adv=true';

        var areOpFeaturesEnabled = ($('#hfAreOpFeaturesEnabled').val() || 'false').toLowerCase() === 'true';
        var rawSearchUrl = ($('.js-custom-queryBuilder').length > 0 || areOpFeaturesEnabled)
            ? customQueryBuilder
            : defaultQueryBuilder;

        var searchUrl = 'search-results?' + rawSearchUrl;
    ...
    function doSolrSearch(inquiry, searchScope) {
        ...
        if (searchScope === hfjournalSiteScope) { //Search is done across all journals.
            ...
            searchUrl = getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: searchSiteID, passAllJournals: true });
            if (hfSiteURL.length) {
                searchUrl = "//" + hfSiteURL.val() + '/' + searchUrl;
            }
            window.location.href = searchUrl;
        } else if (searchScope === hfparentSiteScope) { //Parent Site
            searchUrl = "//" + hfParentSiteURL + '/' + getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: hfparentSiteID });
            window.location.href = searchUrl;
        } else if (searchScope === hfIssueSiteScope) {  //In Issue Search
            searchUrl = getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: searchSiteID, issueSearch: true });
            if (hfSiteURL.length) {
                searchUrl = "//" + hfSiteURL.val() + '/' + searchUrl;
            }
            window.location.href = searchUrl;
        } else if (searchScope === hfCurrentBookScope) {  //In Current Book
            searchUrl = getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: searchSiteID, currentBookSearch: true });
            if (hfSiteURL.length) {
                searchUrl = "//" + hfSiteURL.val() + '/' + searchUrl;
            }
            window.location.href = searchUrl;
        } else if (searchScope === hfBookSiteScope) { // Search is done across all books.
            searchUrl = "/../books/" + hfdefaultSearchURL + queryText + "&SearchSourceType=1";
            window.location.href = searchUrl;
        } else if (searchScope === hfUmbrellaScope) { // Umbrella.
            searchUrl = "//" + hfUmbrellaSiteUrl + '/' + getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: hfUmbrellaSiteId });
            window.location.href = searchUrl;
        } else if (searchScope.startsWith(hfSeriesScope)) { //Series Taxonomy
            searchUrl = getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: searchSiteID, series: currentSearchScope });
            if (hfSiteURL.length) {
                searchUrl = "//" + hfSiteURL.val() + '/' + searchUrl;
            }
            window.location.href = searchUrl;
        } else if (searchAcrossOA) { //Oxford Taxonomy umbrella search 
            searchUrl = getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: hfUmbrellaSiteId, taxonomy: searchScope });
            if (hfSiteURL.length) {
                searchUrl = "//" + hfUmbrellaSiteUrl + '/' + searchUrl; // + boostQueryParm; //new
            }
            window.location.href = searchUrl;
        } else { //Taxonomy 
            searchUrl = getSearchUrl({ baseUrl: hfdefaultSearchURL, queryText: queryText, searchSiteId: searchSiteID, taxonomy: searchScope });
            if (hfSiteURL.length) {
                searchUrl = "//" + hfSiteURL.val() + '/' + searchUrl; // + boostQueryParm; //new
            }
            window.location.href = searchUrl;
    ...
    function getSearchUrl(params) /* -> string */ {
        
        var searchUrl = params.baseUrl;
        if (params.queryText) {
            searchUrl += params.queryText;
        } else {
            searchUrl = searchUrl.replace('&q=', '');
        }
        if (params.searchSiteId) {
            searchUrl += "&fl_SiteID=" + params.searchSiteId;
        }
        searchUrl += "&SearchSourceType=1";
        if (params.passAllJournals) {
            searchUrl += "&allJournals=1";
        }
        if (params.issueSearch) {
            if (hfIssueSearch) {
                searchUrl += hfIssueSearch;
            }
        }
        if (params.currentBookSearch) {
            if (hfCurrentBookSearch) {
                searchUrl += hfCurrentBookSearch;
            }
        }
        if (params.series) {
            searchUrl += '&' + params.series;
        }
        if (params.taxonomy) {
            searchUrl += '&' + getTaxonomyParameter(params.taxonomy);
        }
        return searchUrl;
    }
    ...
    function buildSaveSearchUiEvent() {
        ...
        lnkSaveSolrSearch.off("click").on("click", function () {
        ...
        btnSolrSearchSave.off("click").on("click", function () { saveSolrSearchInfo(solrSearchSaveModel); });
    ...
    function initCollectionArrows() {
        if (SCM.SolrSearch.ExpandedCollectionSites
            && SCM.SolrSearch.ExpandedCollectionSites.length > 0) {
            for (i = 0; i < SCM.SolrSearch.ExpandedCollectionSites.length; i++) {
                var siteId = SCM.SolrSearch.ExpandedCollectionSites[i];
                var expandedIcon = $(".js-collection-facet").find('.js-collection-arrow-icon[data-siteid=' + siteId + ']');
                if (expandedIcon && expandedIcon.hasClass('icon-general-arrow-filled-right')) {

                    expandedIcon.removeClass('icon-general-arrow-filled-right');
                    expandedIcon.addClass('icon-general-arrow-filled-down');
                    $('.js-collection-headings-placeholder[data-siteid=' + siteId + ']').toggleClass('hide');
                }
            }
        }
    }
    ...
    function restrictedImagesSearch() {
        $('div.sri-figure[restricted]')
            .each(function () {
                $(this).off('contextmenu').on('contextmenu', function () { return false; });
            });
    }
    ...
    $('body').on('click', '.js-taxonomy-label', function () {
        var $el = $(this);
        var curTaxonomyNodeId = "";
        var taxEqualsKey;
        var taxKey;
        if ($el.hasClass('js-taxonomy-checkbox')) {
            taxEqualsKey = 'taxWithOr=';
            taxKey = 'taxWithOr';
             // check or uncheck all children
            var $parentContainer = $(this).closest('.js-taxonomy-child-list-item');
            if ($(this).is(":checked")) {
                $parentContainer.find('.js-taxonomy-checkbox').prop('checked', true);
            }
            else {
            ...
            var $selectedTaxIDs = $(this).closest('.js-taxonomy-tree-wrap').find('.js-taxonomy-checkbox:checked');
            $selectedTaxIDs.each(function (index) {
                curTaxonomyNodeId += $(this).attr('data-id') + ',';
            });
            if (curTaxonomyNodeId.endsWith(',')) {
                curTaxonomyNodeId = curTaxonomyNodeId.slice(0, -1);
            }
        }
        else {
            taxEqualsKey = 'tax=';
            taxKey = 'tax';
            curTaxonomyNodeId = $(this).parent('.taxonomy-label-wrap').attr('data-id');
        }
        var currentDataUrl = $('#TaxonomyCurrentDataUrl').val();
        currentDataUrl = decodeURIComponent(currentDataUrl);

        var params = currentDataUrl.split('&');
       // Remove existing tax query and re-add later
        params = $.grep(params, function (item, i) {
            return !item.startsWith(taxEqualsKey);
        });        
        ...
        var taxQsPos = currentDataUrl.indexOf(taxEqualsKey);
        var taxQsPresent = (taxQsPos === -1) ? false : true;

        if (!taxQsPresent || taxKey === "taxWithOr") {
            //filter applied for the first time.
            currentDataUrl = params.join('&').concat('&' + taxEqualsKey + curTaxonomyNodeId);

        } else {
            //taxonomy filter applied previously
            var taxQueryTerm = $.deparam.querystring()[taxKey];
            if (taxQueryTerm && taxQueryTerm !== "") {
                ...
                if (taxQueryTerm.indexOf("=" + rootNodeId) > -1) {
                    ...
                    currentDataUrl = currentDataUrl.replace(taxEqualsKey + taxQueryTerm, taxEqualsKey + curTaxonomyNodeId);

                } else if ($.inArray(curTaxonomyNodeId, decodeURIComponent(taxQueryTerm
                                                                                .slice(4))
                                                                                .split(',')) === -1) {
    ...
    bodyVar.on('click', '.js-collection-arrow', function (e) {
        var $icon = $(this).find('.js-collection-arrow-icon');
        if (!$icon.length) {
            $icon = $(this).parent().find('.js-collection-arrow-icon');
        }
        if (!$icon.length) {
            console.error('.js-collection-arrow-icon not found');
            return;
        }
        var siteId = $icon.data('siteid');
        if (!siteId) {
            console.error('siteid not found');
            return;
        }
        var $placeholder = $('.js-collection-headings-placeholder[data-siteid=' + siteId + ']');
        if (!$placeholder.length) {
            console.error('.js-collection-headings-placeholder not found');
            return;
        }

        if ($icon.hasClass('icon-general-arrow-filled-right')) {

            if (SolrSearch.ExpandedCollectionSites.indexOf(siteId) < 0) {
                SolrSearch.ExpandedCollectionSites.push(siteId);
            }
            ...
            SolrSearch.ExpandedCollectionSites.splice(SolrSearch.ExpandedCollectionSites.indexOf(siteId), 1);
    ...
    bodyVar.on('click', '.js-collection-label', function (e) {
        if ($(e.target).is('.js-collection-checkbox')) {
            return;
        }

        var $checkboxes = $(this).closest('.js-collection-item').find('.js-collection-checkbox');

        // Check or uncheck the clicked checkbox
        var $clickedCheckbox = $checkboxes.first();
        ...
        updateCollectionFacetDescendantCheckboxes($checkboxes);
    ...
    function updateCollectionFacetDescendantCheckboxes($checkboxes) {
        // Check or uncheck the descendant checkboxes
    ...
    function doCollectionFacetQuery() {
        var currentDataUrl = $('#CollectionCurrentDataUrl').val();
        currentDataUrl = decodeURIComponent(currentDataUrl);
        var selectedCollections = $(".js-collection-facet").find('.js-collection-checkbox:checked');

        var siteIDs = [];
        for (i = 0; i < selectedCollections.length; i++) {
            var collection = selectedCollections[i];
            var id = collection.dataset.siteid;
            siteIDs.push(id);
        }

        var collectionQsPos = currentDataUrl.indexOf("collection");
        var collectionQsPresent = (collectionQsPos === -1) ? false : true;

        if (!collectionQsPresent && siteIDs.length > 0) {
            currentDataUrl += "&collection=" + siteIDs.join(",");
        } else if (collectionQsPresent) {

            var collectionQueryTerm = $.deparam.querystring()["collection"];
            if (collectionQueryTerm && collectionQueryTerm !== "") {

                if (siteIDs.length > 0) {
                    var newQuery = "collection=" + siteIDs.join(",");
                    currentDataUrl = currentDataUrl.replace("collection=" + collectionQueryTerm, newQuery);
                } else {
                    var exitingQuery = "collection=" + collectionQueryTerm;
                    var startIndex = currentDataUrl.indexOf(exitingQuery);
                    var endIndex = startIndex + exitingQuery.length;

                    if (currentDataUrl.charAt(startIndex - 1) === "&") {
                        exitingQuery = "&" + exitingQuery;
                    } else if (currentDataUrl.charAt(endIndex + 1) === "&") {
                        exitingQuery = exitingQuery + "&";
                    }

                    currentDataUrl = currentDataUrl.replace(exitingQuery, "");
                }
            }
        }
    ...
    SolrSearch.init = function () {
        ...
        // If view is ViewLarge this function will break MathJax, therefore we will not execute this on ViewLarge
        if (SCM.SiteJS.isPageViewLarge()) {
            return;
        }
        ...
        bodyVar.on('click', '.sf-group-header', function () {
            ...
            if ($(window).outerWidth() < 1024) {
                ...
                $this.siblings('.taxonomy-child-list').slideToggle();
...
(function(iou, $, scmLocalStorage, undefined) {
    ...
    iou.replaceOpenUrlPlaceHolders = function ($openUrlPlaceHolderElements, replaceLinkHtml) {
        if (replaceLinkHtml && $openUrlPlaceHolderElements) {
            $openUrlPlaceHolderElements.each(function () {
                var $placeholder = $(this);
                var targetId = $placeholder.attr('data-targetId');
                var finalLinkHtml = replaceLinkHtml.replace('{openUrlTargetId}', targetId);
                $placeholder.html(finalLinkHtml);
                $placeholder.parent().show().removeClass('empty-target');
            });
        }
    };

    iou.replaceOpenUrlNonDOIPlaceHolders = function ($openUrlPlaceHoldersNoDOIElements, baseUrl, replaceHtml) {
        if (replaceHtml && $openUrlPlaceHoldersNoDOIElements) {
            $openUrlPlaceHoldersNoDOIElements.each(function () {
                var $placeholder = $(this);
                var $target = $placeholder.find(".js-open-url-link");
                if ($target.length) {
                    var targetUrlTemplate = $target.attr('data-href-template');
                    if (targetUrlTemplate && targetUrlTemplate.length) {
                        var targetUrl = targetUrlTemplate.replace('{targetURL}', baseUrl);
    ...
    iou.getAndReplaceOpenUrlPlaceHolders = function ($openUrlPlaceHolderElements, $openUrlPlaceHoldersNoDOIElements) {
        ...
        $.ajax({
            ...
            success: function (resultObject) {
                ...
                if (resultObject.DisplayOpenUrlLink) {
                    if (resultObject.LinkImageSource) {
                        replacelink = " <a class='openInAnotherWindow' target='_blank' href='" + resultObject.TargetUrl + "'><img alt='OpenURL Link' src='" + resultObject.LinkImageSource + "'/></a> ";
                        nonDOIreplaceLink = "<img alt='OpenURL Link' src='" + resultObject.LinkImageSource + "'/>";
                    } else {
                        replacelink = " <a class='openInAnotherWindow' target='_blank' href='" + resultObject.TargetUrl + "'> " + resultObject.LinkImageText + " </a> ";
                        nonDOIreplaceLink = "<img alt='OpenURL Link' src='" + resultObject.LinkImageSource + "'/> ";
                    }
                }
                ...
                if (scmLocalStorage
                    ...
                    && window.App.LoginUserInfo
                    && window.App.LoginUserInfo.currentSessionId) {

                    scmLocalStorage.setItem(instOpenUrlKey + window.App.LoginUserInfo.currentSessionId,
                                            replacelink,
                                            window.sessionStorage);
                    scmLocalStorage.setItem(instOpenUrlNoDOIKey + window.App.LoginUserInfo.currentSessionId,
                                            nonDOIreplaceLink,
                                            window.sessionStorage);
                    scmLocalStorage.setItem(instOpenUrlBaseUrlKey + window.App.LoginUserInfo.currentSessionId,
                                            resultObject.BaseUrl,
                                            window.sessionStorage);
                }
            },
    ...
    iou.checkAndReplaceOpenUrlPlaceHolders = function () {
        ...
        if (window.App.LoginUserInfo && window.App.LoginUserInfo.isInstLoggedIn > 0) {
            // check if article has any openurl templates wtih dois
            var $openUrlPlaceHolders = $('.inst-open-url-holders');
            ...
            var $openUrlPlaceHoldersNoDOI = $('.js-inst-open-url-holders-nodoi');
            // DOing this to simplify checks for null/undefined later 
            var $openUrlNoDOIs = (
            ...
            var storedLink = null;
            if (scmLocalStorage && window.sessionStorage && window.App.LoginUserInfo.currentSessionId) {
                //if we have one, we should have the other
                var storedLink = scmLocalStorage.getItem(instOpenUrlKey + window.App.LoginUserInfo.currentSessionId, window.sessionStorage);
                var storedNoDOILink = scmLocalStorage.getItem(instOpenUrlNoDOIKey + window.App.LoginUserInfo.currentSessionId, window.sessionStorage);
                var storedUrl = scmLocalStorage.getItem(instOpenUrlBaseUrlKey + window.App.LoginUserInfo.currentSessionId, window.sessionStorage);
            }
...
;(function(_cookiepolicyfancybox) {
		...
		function writeTheElements(){
			...
			element.innerHTML = "<div id=\"cookiepolicy_div\"><div id=\"cookiepolicy_parent\"style=\"display:none\"><div id=\"cookiepolicy_data\"></div></div></div>";
...
(function (ns, $) {
    ...
    ns.registerHeadingsClick = function () {
        $(document).on('click', '.js-show-headings-button', onHeadingsClick);

        if ($('#hfIsBookTocSlim').val()) {
            $(document).on('click', '.js-tocLink', onHeadingsClick);
        }
    };

    ns.checkAccess = function () {        
        if ($('.chapter-user-restricted').length) {
            var $activeLink = $('.js-tocLink.active');
            if ($activeLink.length) {
                var $chapterLink = $activeLink.closest('.js-toc-chapter-link');
                // turn off the chapter subheading links and make them spans
                $chapterLink.find('.js-jumplink').addClass('hide').removeAttr('href');
                $chapterLink.find('.js-jumplink-span').removeClass('hide');
            }
        }
    };

    /**
    * manually add the Notes link in the Chapter TOC if end notes widget exists and has data
    */
    ns.addNotesTocLink = function () {
        var $endNotes = $("#endNotesTargetLocation").find(".widget-items");
        var $footNotes = $("#chapter-footnotes");
        var $chapterLink = $('.js-tocLink.active:last');
        var $chapterSections = $chapterLink.closest('.js-toc-chapter-link');
        if ($chapterSections.length) {
            if ($endNotes.length && $endNotes.html().trim() != '') {
                var $endNotesLink = $chapterSections.find('.js-toc-notes-link');
                if ($endNotesLink.length) {
                    $endNotesLink.removeClass('hide');
                    SCM.JumpLinks.registerNotesTocLink("#endNotesTargetLocation");
                }
            }
            if ($footNotes.length) {
                var $footNotesLink = $chapterSections.find('.js-toc-footnotes-link');
                if ($footNotesLink.length) {
                    $footNotesLink.removeClass('hide');
                    SCM.JumpLinks.registerNotesTocLink("#chapter-footnotes");
                }
            }
        }
        else {
            $('.js-show-headings-button').addClass('hide');
        }
    };
            ...
            if (!$placeholder.children().length) {
                ...
                $.ajax({
                    ...
                    data: { 
                        chapterId: sectionId,
                        targetUrl: targetUrl,
                        BypassOutputCache: IsBypassOutputCacheEnabled()
                    }
    ...
    function IsBypassOutputCacheEnabled() /* -> boolean */ {
        return window.location.search.toLowerCase().indexOf("bypassoutputcache=true") > -1;
    }
    ...
    ns.goToPageHandler = function () {
        $("#goToPageNumberBook, #goToPageNumber").on('click', function (e) {
            ...
            if (pageNumber.length) {
                ...
                // If it's on this page and not a page on the current chaper page, don't need 
                // to find the chapter on the server side
                var $pageMatch = $("a[name='page-" + pageNumber + "']").closest("#endNotesTargetLocation");

                if (!$pageMatch.length&& $page && $page.length) {
...
(function (ns, $) {
    ns.initChapterAbstractClickEvent = function() {
        $(".master-main").on('click', ".js-showChapterAbstractLink", function () {
            ...
            var chapterId = $(wrapperDiv).attr('data-chapterid-for-abstract');
            var bookId = $(wrapperDiv).attr('data-bookid-for-abstract');
            var triedFetchDiv = $(wrapperDiv).children('div[data-tried-to-fetch-abstract]').first();
            var triedFetch = $(triedFetchDiv).attr("data-tried-to-fetch-abstract") === "true";
            var abstractLocation = $('#abstract-' + chapterId);
            var baseSiteUrl = $("#hfSiteURL");
            var abstractLinks = $('#abstract-' + chapterId + '-links');
            var ajaxUrl = $(this).parent().attr('data-link-chapter-abstract-type') == 'extract' ? "/PlatformBook/ChapterAbstractOrExtractAjax" : "/PlatformBook/ChapterAbstractAjax";

            if (baseSiteUrl && baseSiteUrl.val()) {
                ajaxUrl = "//" + baseSiteUrl.val() + ajaxUrl;
            }
            ...
            var toggleIconClass = $thisAbstractIcon.attr('data-icon-class');
            var downArrow = toggleIconClass + '-down';
            var upArrow = toggleIconClass + '-up';
...
(function (ns, $) {
    ns.initBookAbstractClickEvent = function () {
        $(".master-main").on('click', ".js-showBookAbstractLink", function () { 
            ...
            var bookId = $(wrapperDiv).attr('data-bookid-abstract');
            var triedFetchDiv = $(wrapperDiv).children('[data-tried-to-fetch-abstract]').first();
            var triedFetch = $(triedFetchDiv).attr("data-tried-to-fetch-abstract") === "true";
            var abstractLocation = $('#abstract-' + bookId);
            var baseSiteUrl = $("#hfSiteURL");
            var abstractLinks = $('#abstract-' + bookId + '-links');
            var ajaxUrl = $(this).parent().attr('data-link-book-abstract-type') == 'extract' ? "/Book/BookAbstractOrExtractAjax" : "/Book/BookAbstractAjax";
...
$(function () {
    ...
    if (enableOnlineFeatures && enableOnlineFeatures.val() === 'True') {
        ...
        $('.js-citation-links').on('click', function (e) {
            e.stopPropagation();
            var $siblings = $(this).siblings('.citation-wrap');
            if ($siblings.is(':visible')) {
                $siblings.removeClass('show-inline').addClass('hidden');
            }
            else {
                // close any other open ones first
                $('.citation-wrap').removeClass('show-inline').addClass('hidden');
                $siblings.removeClass('hidden').addClass('show-inline');
            }
        });
        $(document).on('click', function () {
            $('.citation-wrap').removeClass('show-inline').addClass('hidden');
...
(function (ns, $) {
    ...
    ns.queryBuilderHandler = function () {
        var queryString = "";
        var currentDataUrl = $('.querybuilder-submit').attr('data-current-url');
        var oldQueryParams = currentDataUrl.split('&');
        // strip out params since we're starting a new query and cqb
        var paramsToRemove = ['fd_IsLatestEdition', 'cqb', 'qb', 'page', 'q'];

        var queryParams = oldQueryParams.filter(function (el) {
            var paramType = el.split('=')[0];
            return (paramsToRemove.indexOf(paramType) < 0);
        });

        queryParams = buildQB(queryParams);

        queryParams.push("page=1");
        var param = queryParams.filter(function (el) {
            return el.startsWith("searchType");
        });
        if (!param) {
            queryParams.push("searchType=advanced");
        }
        param = queryParams.filter(function (el) {
            return el.startsWith("adv");
        });
        if (!param) {
            queryParams.push("adv=true");
        }
    ...
    function buildQB(queryParams) {
        ...
        $('.querybuilder-wrap .querybuilder-instance').each(function (i) {
            ...
            var instanceVisible = !$this.hasClass('hide');
            if (instanceVisible) {
                var keyOperatorOrdinal = "qOp" + instanceIndex;
                // add conditional to the group (if exists)
                if ($operator !== undefined && $operator.length > 0) {
                    qbObject[keyOperatorOrdinal] = $operator.val();
                }

                //find the search term 
                var $qbInputElement = $this.find('#instance_QueryBuilderSearchTerm');
                var qbInputElementVal;
                // If the default behaviour failed, fall back to OUP-specific class-name
                if ($qbInputElement.length === 0) {
                    qbInputElementVal = $this.find('.querybuilder-input').val().trim();
                }
                else {
                    qbInputElementVal = $qbInputElement.val().trim();
                }
                var qbSearchTerm = qbInputElementVal.replace(/^"(.*)"$/, '$1');

                var searchTerm = encodeURIComponent(qbSearchTerm);

                //get the text and value for the dropdown item
                //var qbSearchTypeDisplayName = $this.find('.querybuilder-filter-dropdown li.selected').text();
                var qbSearchTypeFieldName = $this.find('.querybuilder-filter-dropdown .querybuilder-filter.selected').attr('data-fieldname');
                if (qbSearchTypeFieldName === undefined || qbSearchTypeFieldName.length === 0) {
                    qbSearchTypeFieldName = '';
                }

                var term = {};
                $operator = $this.find('[name^=operator-]:checked');
                // add conditional to the group (if exists)
                if ($operator !== undefined && $operator.length > 0) {
                    // if OR, create a new grouping
                    if ($operator.val() === "OR") {
                        if (terms.length) {
                            // ensures that the first term in a group does not contain the condition property
                            if (terms[0].condition) {
                                delete terms[0].condition;
                            }
                            group["terms"] = terms;
                            jsonObj.push(group);
                            terms = [];
                        ...
                        group['condition'] = $operator.val();
                    }
                    else {
                        term['condition'] = $operator.val();
                    }
                    qbObject[keyOperatorOrdinal] = $operator.val();
                }
                // special case for the ALL filter
                if (qbSearchTypeFieldName === 'all') {
                    term['filter'] = '';
                }
                else {
                    term['filter'] = qbSearchTypeFieldName;
                }
                term['input'] = searchTerm;

                // If it has quotes, it's an exact match
                if (/[^']*'$/.test(qbInputElementVal) || /[^"]*"$/.test(qbInputElementVal)) {
                    term['exactMatch'] = true;
                }
                
                if (term.input && term.input.length) {
                    terms.push(term);
                }
                if (qbInputElementVal.trim() !== "") {
                    var key = qbSearchTypeFieldName + instanceIndex;
                    if (qbSearchTypeFieldName === "") {
                        // No filter selected
                        key = "q";
                        if (instanceIndex !== 1) {
                            key = key + instanceIndex;
                        }
                    }
                    qbObject[key] = searchTerm;
                    qbObjectInitialized = true;
                }
            }
            instanceIndex++;
        ...
        if (terms.length) {
            // ensures that the first term in a group does not contain the conditon property
            if (terms[0].condition) {
                delete terms[0].condition;
            }
            group["terms"] = terms;
        ...
        // ensures that the first group in the obj does not contain the condition property
        if (jsonObj.length) {
            if (jsonObj[0].condition) {
                delete jsonObj[0].condition;
            }
        }

        var jsonObjStr = JSON.stringify(jsonObj);
        queryParams.push("cqb=" + jsonObjStr);
        if (qbObjectInitialized) {
            var qbObjectParmsStr = JSON.stringify(qbObject);
            queryParams.push("qb=" + qbObjectParmsStr);
        }
...
// ----
// JS for SCM Pagination.
// ----

var SCM = SCM || {};

SCM.Pagination = (function ($)
{
    return {
        init: init,
        performAjaxPagination: performAjaxPagination,
        updateBrowseHistory: updateBrowseHistory,
        enableBrowserHistoryNavigation: enableBrowserHistoryNavigation,
        trimEndUntilRegexMatches: trimEndUntilRegexMatches,
        scrollToTop: scrollToTop
    };

    function init() {
        if ($('.js-pagination-click').length) {
            $('body').on('click',
                '.js-pagination-click',
                function () {
                    var clickedElem = $(this);

                    var handler = clickedElem.data("click-handler");
                    var parts = handler.split(".");

                    //window[App or SCM][WidgetName][functionName]
                    window[parts[0]][parts[1]][parts[2]](clickedElem);
                });
        }
    }
    ...
    // Assumes the click handler has created the ajaxSettings, and that the clicked Element has been passed back
    function performAjaxPagination(ajaxSettings, clickedElem) {
        var elemLocatorString = clickedElem.data("elem-to-replace");

        if (elemLocatorString) {
            var elementToReplace = $(clickedElem.data("elem-to-replace"));

            if (elementToReplace) {
                $.ajax(ajaxSettings)
                    .done(function(html) {
                        elementToReplace.html(html);
                    })
                    .fail(function(xmlHttpRequest, textStatus, errorThrown) {
                        if (window.console) {
                            // this really helps with debugging during development
                            console.log("error: " + xmlHttpRequest.responseText);
                            console.log("status: " + textStatus);
                        };
                    });
            } else {
                alert("Unable to find an element to respond to ajax response.");
            }
        } else {
            alert(
                "The elem-to-replace data attribute was not populated.  You must populate this in the Widget's view to pass back the call to performAjaxPagination");
        }
    }

    // baseUrl is the url of the page if the page number is not included
    // newUrl includes the page number of the pagination element that was just clicked
    function updateBrowseHistory(baseUrl, newUrl) {
        var state = { pagination: newUrl };

        if (history.pushState) {

            // handles the first load of the page
            if (history.state === null || !history.state.pagination) {
                history.replaceState({ pagination: document.URL}, null, document.URL);
            }
    ...
    // Adds an event listener that will cause the page to reload when pressing the back (or fwd) button.  Only use if you've been updating the browser history
    function enableBrowserHistoryNavigation() {
        window.addEventListener('popstate', function (e) {
            if (e.state && history.state.pagination) {
                window.location = e.state.pagination;
            }
        });
    }

    // convenience method to help trim the baseUrl (in cases where pagination adds a page number to the url)
    function trimEndUntilRegexMatches(url, regex) {
        while (!url.match(regex)) {
            url = url.substr(0, url.length - 1);

            if (url.length === 0) {
                alert("SCM.Pagination.trimEndUntilRegexMatches: " +
                    "The entire string was trimmed, this likely indicates a problem with your regex pattern.  " +
                    "Note:  This method expects a regular expression literal (see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions)");
                return "";
            }
        }
...
SCM.CitedByPagination = (function ($) {
    ...
    function init() {
        if ($(".widget-CrossRefCitedBy").length) {
            baseUrl = SCM.Pagination.trimEndUntilRegexMatches(document.URL.valueOf(), /[a-zA-Z]\/\d+$/);
            articleId = getArticleIdFromUrl(baseUrl);

            SCM.Pagination.enableBrowserHistoryNavigation();
        }
    }

    function getArticleIdFromUrl(baseUrl) {
        return baseUrl.match(/[a-zA-Z]\/(\d+)$/)[1];
    }

    function handlePaginationClick(jqElement) {
        SCM.Pagination.updateBrowseHistory(baseUrl, baseUrl + "/" + jqElement.data("clicked-page"));

        var ajaxSettings = {
            type: "POST",
            url: (App.CurrentSubdomain.length ? "/" + App.CurrentSubdomain : "") +
                "/ThirdParty/CrossRefCitedBy/" +
                jqElement.data("parent-instance"),
            cache: false,
            data: {
                selectedPage: jqElement.data("clicked-page"),
                articleId: articleId
            }
        };

        SCM.Pagination.performAjaxPagination(ajaxSettings, jqElement);
    }
